<template>
  <div class="main-container-data">
    ...
    <div class="card-data">
      ...
      <div class="inside-card">
        <img class="img-control" src="../assets/images/puerta.png" />
        <h3>Porton</h3>
        <form action="">
          <label class="switch">
            <input type="checkbox" v-model="gateStatus" @change="openGate" />
            <span class="slider round"></span>
          </label>
        </form>
      </div>
  ...
  methods: {
    fetchData() {
      fetch("")
        ...
        .then((data) => {
          this.humedadData = data[0].Presencia;
          this.temperaturaData = data[0].Distancia;
          console.log(data);
    ...
    fetchGateStatus() {
      fetch("http://192.168.0.112:3000/consults/garaje/gate")
        .then((response) => response.json())
        .then((data) => {
          this.windowStatus = data[0].isOpened;
        })
        .catch((error) => {
          console.error("Error fetchGateStatus():", error);
        });
    },
    fetchLightStatus() {
      fetch("http://192.168.0.112:3000/consults/garaje/internal-light")
        .then((response) => response.json())
        .then((data) => {
    ...
    fetchDoorStatus() {
      fetch("http://192.168.0.112:3000/consults/garaje/door")
        .then((response) => response.json())
        .then((data) => {
